...
export const brandRoutes = (app: Express) => {
  ...
  app.get('/api/brands', authenticate, requirePermission('brand:read'), listBrands);

  // GET /api/brands/:id - Get single brand by ID (requires permission: brand:read)
  app.get('/api/brands/:id', authenticate, requirePermission('brand:read'), getBrand);

  // POST /api/brands - Create new brand (requires permission: brand:create)
  app.post('/api/brands', authenticate, requirePermission('brand:create'), createBrandController);

  // PUT /api/brands/:id - Full update of brand (requires permission: brand:update)
  app.put('/api/brands/:id', authenticate, requirePermission('brand:update'), updateBrandController);

  // PATCH /api/brands/:id - Partial update of brand (requires permission: brand:update)
  app.patch('/api/brands/:id', authenticate, requirePermission('brand:update'), partialUpdateBrandController);

  // DELETE /api/brands/:id - Delete brand (requires permission: brand:delete)
  app.delete('/api/brands/:id', authenticate, requirePermission('brand:delete'), deleteBrandController);

  // POST /api/brands/:brandId/media - Assign media (logo) to brand (requires permission: brand:update)
  app.post('/api/brands/:brandId/media', authenticate, requirePermission('brand:update'), assignBrandMediaController);

  // DELETE /api/brands/:brandId/media - Remove media (logo) from brand (requires permission: brand:update)
  app.delete('/api/brands/:brandId/media', authenticate, requirePermission('brand:update'), removeBrandMediaController);
};
