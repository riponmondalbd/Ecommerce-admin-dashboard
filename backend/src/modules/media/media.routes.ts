...
export const mediaRoutes = (app: Express) => {
  ...
  app.get('/api/media', requirePermission('media:read'), listMedia);

  // GET /api/media/:id - Get single media by ID (requires permission: media:read)
  app.get('/api/media/:id', requirePermission('media:read'), getMediaController);

  // POST /api/media - Upload new media (requires permission: media:create)
  app.post('/api/media', requirePermission('media:create'), upload.single('file'), createMediaController);

  // PUT /api/media/:id - Full update of media (requires permission: media:update)
  app.put('/api/media/:id', requirePermission('media:update'), updateMediaController);

  // PATCH /api/media/:id - Partial update of media (requires permission: media:update)
  app.patch('/api/media/:id', requirePermission('media:update'), partialUpdateMediaController);

  // DELETE /api/media/:id - Delete media (requires permission: media:delete)
  app.delete('/api/media/:id', requirePermission('media:delete'), deleteMediaController);

  // GET /api/media/assigned-to/:userId - Get users' own media (requires media:read)
  app.get('/api/media/assigned-to/:userId', requirePermission('media:read'), getUserMediaController);
};
