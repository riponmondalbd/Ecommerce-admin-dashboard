...
export const UpdateProductDto = z
  .object({
    ...
    mediaIds: z.array(z.string()).optional().nullable(),
  })
  .refine(
    (data) => data.salePrice === undefined || data.salePrice === null || data.price === undefined || data.price === null || data.salePrice <= data.price,
    { message: 'Sale price must not exceed price', path: ['salePrice'] },
  );
